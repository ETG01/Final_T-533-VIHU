...
test.describe("Gameplay", () => {
  ...
  test("Gameplay Your Name Wins", async ({ page }) => {
    ...
    await page.waitForSelector('h1:has-text("Loading..🔃")', {
      state: "detached",
    });
    // 0 1 2
    // 3 4 5
    ...
    for (const { player, cell } of moves) {
      await page.waitForSelector(`h1:has(div:has-text("${player}"))`, {
        timeout: 20000,
      });
...
test.describe("All games List", () => {
  test("From / to Check Game List", async ({ page }) => {
    ...
    await page.waitForSelector("div.Row_gameInfo__F31_v", {
      state: "attached",
    });
    ...
    const gameRowsHTML = await Promise.all(
    ...
    expectedGameRowHTMLs.forEach((expectedHTML) => {
      const isPresent = gameRowsHTML.some((gameRowHTML) =>
        gameRowHTML.includes(expectedHTML),
      );
    ...
    for (const gameRow of gameRows) {
      ...
      const boardCells = await gameRow.$$(
        `div.Board_container__VfdJF div.Cell_miniCell__GEFD3`,
      );
